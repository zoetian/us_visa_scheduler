...
SLEEP_TIME = 300  # recheck time interval: 20 seconds
EXCEPTION_TIME = 60*1  # recheck exception time interval: 5 minutes
RETRY_TIME = 60*60  # recheck empty list time interval: 60 minutes

DATE_URL = f"https://ais.usvisa-info.com/en-ca/niv/schedule/38603652/appointment/days/{DAYS_IN_COUNTRY}.json?appointments[expedite]=false"
TIME_URL = f"https://ais.usvisa-info.com/en-ca/niv/schedule/38603652/appointment/times/{DAYS_IN_COUNTRY}.json?date=%s&appointments[expedite]=false"
APPOINTMENT_URL = f"https://ais.usvisa-info.com/en-ca/niv/schedule/38603652/appointment"
EXIT = False
...
def login():
    # Bypass reCAPTCHA
    driver.get(f"https://ais.usvisa-info.com/en-ca/niv")
    ...
    while 1:
        ...
        try:
            ...
            if not dates:
              ...
              SLEEP = random.randint(1, 5) * 60
              print(f"gonna SLEEP {SLEEP} second")
              time.sleep(SLEEP)

        except:
            retry_count += 1
            EXCEPTION_TIME = random.randint(2, 5) * 60
            print(f"EXCEPTION_TIME {EXCEPTION_TIME} second")
